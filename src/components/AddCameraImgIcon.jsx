
// let AddCameraImageIcon = ({ func, className }) => {
//     return (

//         <label htmlFor="file-input" className="sm:py-[16px] py-[8px] sm:block hidden">
//             <button onClick={func} className={`cursor:pointer ${className}`}>
//                 <svg width="60px" height="60px" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
//                     <g id="icomoon-ignore">
//                     </g>
//                     <path d="M27.195 9.069h-4.41l-2.564-2.57c-0.003-0.003-0.006-0.005-0.008-0.008l-0.005-0.004c-0.385-0.381-0.914-0.616-1.499-0.616h-5.331c-0.62 0-1.172 0.268-1.562 0.689l-0.002-0.006-2.511 2.516h-4.5c-1.178 0-2.132 0.955-2.132 2.133v12.795c0 1.178 0.955 2.133 2.132 2.133h22.391c1.178 0 2.133-0.955 2.133-2.133v-12.795c-0-1.178-0.955-2.133-2.133-2.133zM28.262 23.997c0 0.587-0.479 1.066-1.067 1.066h-22.391c-0.588 0-1.066-0.479-1.066-1.066v-12.795c0-0.588 0.479-1.066 1.066-1.066h4.942l1.604-1.607 0.026 0.078 1.224-1.324c0.207-0.224 0.484-0.347 0.779-0.347h5.331c0.282 0 0.548 0.109 0.757 0.316l2.876 2.883h4.852c0.588 0 1.067 0.478 1.067 1.066v12.795z" fill="#6E6F71">

//                     </path>
//                     <path d="M16 10.136c-3.828 0-6.931 3.102-6.931 6.93s3.103 6.932 6.931 6.932 6.931-3.104 6.931-6.932-3.103-6.93-6.931-6.93zM16 22.931c-3.234 0-5.864-2.631-5.864-5.865s2.631-5.863 5.864-5.863 5.864 2.63 5.864 5.863c0 3.234-2.631 5.865-5.864 5.865z" fill="#6E6F71">

//                     </path>
//                     <path d="M16 12.268c-2.651 0-4.798 2.147-4.798 4.797s2.148 4.799 4.798 4.799 4.798-2.149 4.798-4.799-2.148-4.797-4.798-4.797zM16 20.798c-2.058 0-3.732-1.675-3.732-3.733s1.674-3.731 3.732-3.731 3.732 1.673 3.732 3.731-1.674 3.733-3.732 3.733z" fill="#6E6F71">

//                     </path>
//                 </svg>
//             </button>
//             <input
//                 className="mb-[12px] hidden"
//                 ref={hiddenFileInput}
//                 id="file-input"
//                 type="file"
//                 accept="image/*"
//                 capture="camera"
//                 onChange={handleImageUpload}
//             />
//         </label>

//     )
// }

// export default AddCameraImageIcon;

